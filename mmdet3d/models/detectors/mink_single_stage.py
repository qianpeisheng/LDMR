...
class MinkSingleStage3DDetector(Base3DDetector):
    ...
    def extract_feats(self, points):
        ...
        if hasattr(points, 'data'):
            # points is a DataContainer, extract the actual data
            actual_points = points.data
            # DataContainer.data might return nested structure, flatten if needed
            if isinstance(actual_points, list) and len(actual_points) > 0 and isinstance(actual_points[0], list):
                # Flatten nested list structure: [[tensor1, tensor2], ...] -> [tensor1, tensor2, ...]
                flattened = []
                for batch in actual_points:
                    if isinstance(batch, list):
                        flattened.extend(batch)
                    else:
                        flattened.append(batch)
                actual_points = flattened
        else:
            actual_points = points
            
        coordinates, features = ME.utils.batch_sparse_collate(
            [(p[:, :3] / self.voxel_size, p[:, 3:]) for p in actual_points],
            device=actual_points[0].device)
