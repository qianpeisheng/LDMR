...
def _format_eval_banner_tags(eval_context, *, stage_id=None, seen_classes=None):
    """Format eval banner tags for log tables.

    `eval_context` is expected to be a dict with optional keys:
      - dataset: str (e.g. 'SUNRGBD')
      - split: str (e.g. 'val', 'train(memory_bank_subset,...)')
      - purpose: str (e.g. 'epoch_end', 'reviewing')
      - stage_id: int (1-indexed, human stage id)
      - seen_classes: int
      - review_k: int (optional)
    """
    if not isinstance(eval_context, dict):
        eval_context = {}

    tags = ['[EVAL]']
    dataset = eval_context.get('dataset', None)
    if dataset:
        tags.append(f"[{str(dataset)}]")

    split = eval_context.get('split', None)
    if split:
        tags.append(f"[split={split}]")

    purpose = eval_context.get('purpose', None)
    if purpose:
        tags.append(f"[purpose={purpose}]")

    k = eval_context.get('review_k', eval_context.get('k', None))
    if k is not None:
        try:
    ...
    if stage_id is None:
        stage_id = eval_context.get('stage_id', eval_context.get('stage', None))
    if stage_id is not None:
        try:
            tags.append(f"[stage={int(stage_id)}]")
        except Exception:
            pass

    if seen_classes is None:
        seen_classes = eval_context.get('seen_classes', eval_context.get('seen', None))
    if seen_classes is not None:
        try:
            tags.append(f"[seen={int(seen_classes)}]")
        except Exception:
            pass
...
def indoor_eval(gt_annos,
    ...
    ret_dict = dict()
    # Prepare left meta columns: NYU40 id, GCI (class index), Stage id
    # class_meta expected format: {gci: {'nyu40': int, 'stage': int, 'name': str}}
    sorted_labels = sorted(label2cat.keys())
    header = ['NYU40', 'GCI', 'Stage', 'classes']

    nyu_col = []
    gci_col = []
    stage_col = []
    name_col = []
    for gci in sorted_labels:
        meta = class_meta.get(gci, {}) if isinstance(class_meta, dict) else {}
        nyu = meta.get('nyu40', '')
        stg = meta.get('stage', '')
        name = label2cat[gci]
        nyu_col.append(nyu)
        gci_col.append(gci)
        stage_col.append(stg)
        name_col.append(name)
    # Add overall row placeholders
    nyu_col += ['']
    gci_col += ['']
    stage_col += ['']
    name_col += ['Overall']

    # Initialize table columns with left meta columns
    table_columns = [nyu_col, gci_col, stage_col, name_col]
    ...
    for i, iou_thresh in enumerate(metric):
        ...
        ret_dict[f'mAP_{iou_thresh:.2f}'] = float(
            np.mean([ap[i][label][0] for label in ap[i].keys() if label in label2cat]))

        # AP column in sorted label order + Overall
        ap_col_vals = [ap[i][label][0] if label in ap[i] else 0.0 for label in sorted_labels]
        ap_col_vals += [ret_dict[f'mAP_{iou_thresh:.2f}']]
        table_columns.append([f'{float(x):.4f}' for x in ap_col_vals])

        for label in rec[i].keys():
            if label in label2cat:
                ret_dict[f'{label2cat[label]}_rec_{iou_thresh:.2f}'] = float(
                    rec[i][label][-1])
        # Recall column in sorted label order
        rec_list = [rec[i][label][-1] if label in rec[i] else 0.0 for label in sorted_labels]
        ret_dict[f'mAR_{iou_thresh:.2f}'] = float(np.mean(rec_list))
        rec_col_vals = rec_list + [ret_dict[f'mAR_{iou_thresh:.2f}']]
        table_columns.append([f'{float(x):.4f}' for x in rec_col_vals])
    ...
    if eval_context is not None:
        try:
            iou_str = ','.join(f'{float(x):.2f}' for x in metric)
        except Exception:
            iou_str = str(metric)
        banner = _format_eval_banner_tags(
            eval_context,
            seen_classes=len(sorted_labels),
        )
        banner = f"{banner}[scenes={len(gt_annos)}][iou={iou_str}] Per-class AP/AR table:"
