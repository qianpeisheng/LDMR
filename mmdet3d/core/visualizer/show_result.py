...
def _write_oriented_bbox_v2(corners, labels, out_filename):
    ...
    max_label = int(labels_arr.max()) if labels_arr.size > 0 else 0
    num_colors = max(max_label + 1, 18)
    colors = np.multiply([
        plt.cm.get_cmap('nipy_spectral', num_colors + 1)(
            ((i * 5 + 11) % num_colors) + 1)[:3]
        for i in range(num_colors)
    ], 255).astype(np.uint8).tolist()
    with open(out_filename, 'w') as file:
        for i, (corner, label) in enumerate(zip(corners, labels_arr)):
            c = colors[label]
            for p in corner:
