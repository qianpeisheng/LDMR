...
def _project_root() -> str:
    """Resolve repository root for importing config-side mapping modules."""
    import os
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # tools/data_converter -> tools -> repo root
    return os.path.dirname(os.path.dirname(current_dir))


def _load_sunrgbd_top40_raw_classes() -> list:
    import sys
    import os
    ...
    return list(SUNRGBD_40_RAW_TOP40_CLASSES)


SUNRGBD_40_CLASSES = _load_sunrgbd_top40_raw_classes()
...
class SUNRGBDData40Class(object):
    """SUNRGBD data.
    ...
    def __init__(self, root_path, split='train', use_v1=False):
        ...
        self.classes = SUNRGBD_40_CLASSES
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            # NOTE: points are stored as .bin under data_root/points/*.bin.
            # Only generate if missing to avoid re-writing existing sampling.
            SAMPLE_NUM = 50000
            ...
            points_dir = osp.join(self.root_dir, 'points')
            mmcv.mkdir_or_exist(points_dir)
            points_path = osp.join(points_dir, f'{sample_idx:06d}.bin')
            if not osp.exists(points_path):
                pc_upright_depth = self.get_depth(sample_idx)
                pc_upright_depth_subsampled = random_sampling(
                    pc_upright_depth, SAMPLE_NUM)
                pc_upright_depth_subsampled.tofile(points_path)
            ...
            if has_label:
                ...
                raw_names = [
                    (obj.classname or '').strip().lower() for obj in obj_list
                ]
                valid_mask = [n in self.cat2label for n in raw_names]

                annotations = {}
                annotations['gt_num'] = int(np.sum(valid_mask))

                if annotations['gt_num'] != 0:
                    # Filtered & normalized names
                    annotations['name'] = np.array(
                        [n for n, keep in zip(raw_names, valid_mask) if keep])

                    # 2D boxes
                    annotations['bbox'] = np.concatenate(
                        [obj.box2d.reshape(1, 4)
                         for obj, keep in zip(obj_list, valid_mask) if keep],
                        axis=0)
                    ...
                    annotations['class'] = np.array(
                        [self.cat2label[n]
                         for n, keep in zip(raw_names, valid_mask) if keep],
                        dtype=np.int64)

                    # (K, 7): [cx, cy, cz, dx, dy, dz, heading]
                    annotations['gt_boxes_upright_depth'] = np.stack(
                        [obj.box3d
                         for obj, keep in zip(obj_list, valid_mask) if keep],
                        axis=0)

                    # Index aligned with filtered GT count (avoids shape mismatches)
...
# Backward-compatible alias (avoid breaking older scripts importing this name)
SUNRGBDData = SUNRGBDData40Class
