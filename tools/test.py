...
# `python tools/test.py ...` (otherwise `import mmdet3d` fails).
...
def main():
    ...
    # build the dataloader - select dataset based on split argument
    if args.dataset_split == 'train':
        # Build train dataset configuration
        dataset_cfg = cfg.data.train.copy()
        
        # Handle RepeatDataset wrapper (common in training configs)
        if dataset_cfg.get('type') == 'RepeatDataset':
            dataset_cfg = dataset_cfg.dataset
        
        # Set test mode and use test pipeline for evaluation
        dataset_cfg.test_mode = True
        dataset_cfg.pipeline = cfg.data.test.pipeline
        
        dataset = build_dataset(dataset_cfg)
        print(f"Evaluating on TRAINING set ({len(dataset)} samples)")
        ...
        print(f"Evaluating on TEST set ({len(dataset)} samples)")
