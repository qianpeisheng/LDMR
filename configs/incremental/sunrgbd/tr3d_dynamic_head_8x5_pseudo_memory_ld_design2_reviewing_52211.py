"""
SUN RGB-D Incremental Learning (8x5) - LDMR (pseudo + LD Design-2 memory + reviewing)

Final-stage mAP@0.25 = 25.10 (seed 200).

Stage setting:
- `sunrgbd40_s5_freqorder` (the default SUN RGB-D 40-class stage setting)
- Stage sizes: 8 classes per stage
- Dynamic head: 8 -> 16 -> 24 -> 32 -> 40
- Per-stage epochs: 5-2-2-1-1

LDMR components:
- Human-like intra-stage review: 4 review checkpoints -> I = 5 sub-stages
...
# 5-stage setting and stage definitions
stage_setting = 'sunrgbd40_s5_freqorder'
...
reviewing = dict(
    enabled=True,
    # 4 review checkpoints -> I = 5 sub-stages (the paper's default).
